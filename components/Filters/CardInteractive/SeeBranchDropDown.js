...
const SeeBranchDropDown = ({ cardTitle = "", bottomComponent }) => {
  const [cardOpen, setCardOpen] = useState(true);
  const handleOpenClick = () => setCardOpen(!cardOpen);

  return (
    <CardWrapper>
      <div className="bg-[#F3F6F6] rounded-[16px]">
        <FilterTitle handleOpenClick={handleOpenClick} className="flex items-center pt-[6px]">
          <button
            type="button"
            className={`w-3 h-2 text-black transition-transform duration-300 mr-[24px] ${cardOpen ? "" : "rotate-180"}`}
          >
            <svg
              width="14"
              height="8"
          ...
          <span className="text-[#151827] text-[16px] font-semibold">{cardTitle}</span>
        </FilterTitle>
      </div>
      {cardOpen && (
        <div
          className={`px-3 pt-3 ${cardTitle == "Shops" || cardTitle == "Colors" || cardTitle == "Locations" ? "pt-3" : ""} `}
        >
          {bottomComponent}
        </div>
      )}
...
export default SeeBranchDropDown;
