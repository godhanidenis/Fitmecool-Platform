...
const ShowMoreLessFilter = ({ value, onClick }) => {
  return (
    <span
      className="flex items-center justify-center cursor-pointer text-[#29977E] text-base"
      onClick={onClick}
    >
      {value ? "Show More" : "Show Less"}
      <button
        type="button"
        className={`w-3 h-2 ml-2 text-black transition-transform duration-300 ${
          !value ? "" : "rotate-180"
        }`}
      >
        <svg
...
export default ShowMoreLessFilter;
